...
class administracion_usuarios_ajax(View):
    ...
    def post(self, request):
        ...
        estado = request.POST.get('estado_ajax')
        ...
        if estado:
            perfil = perfil.filter(user__is_active=estado)
...
def administracion_usuarios_estado(request):
    ...
    if estado == 'True':
        ...
        html = ('<p style="font-size:12px;">Estimada/o Proveedor</p>'
                '<p style="font-size:12px;">Se activó  su usuario ' + usuario.username + ' correctamente.</p>'
                '<p style="font-size:12px;">Recuerde que no debe presentar ninguna otra documentación.</p>'
        ...
        send_mail(
            'Activación de Usuarios - Sistema de Proveedores.',
            '',
...
def administracion_observacion(request):
    """ VER OBSERVACIONES DEL USUARIO """

    template_name = 'administrador/administrador_observacion.html'
    profile = Profile.objects.get(user=request.POST.get('user'))
    user = User.objects.get(id=request.POST.get('user'))
...
def administracion_actualizar_observacion(request):
    if request.method == "POST":

        profile = Profile.objects.get(pk=request.POST.get('profile'))
        profile.observaciones = request.POST.get('observacion')
        profile.save()
